...
The refusal is deliberately narrow: it fires only when the host canvas is
materially smaller than the result, which is the one case where pixels would
have to be destroyed. A canvas *larger* than the result — the normal shape of a
fixed-size model such as DepthCrafter or NormalCrafter with **Force Size** on —
is scaled up to fill the canvas instead, and never errors. On a fixed-format
host those two plugins previously produced a small frame stranded in black;
they now fill the canvas correctly.
...
- **Autodesk Flame / Flare:** fixed-format host — an OFX node's output
  ...
  plugin, SeedVR2; see
  [the upscale is cropped back to source resolution](#fixed-format-host-crops-upscale). If you hit one, please
