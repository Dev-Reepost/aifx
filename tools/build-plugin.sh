...
show_usage() {
    ...
    echo
    echo "Arguments:"
    echo "  plugin-directory    Path to plugin directory (relative to AIFX root)"
    ...
    echo
    echo "Examples:"
    echo "  $0 plugins/depth_da3 DepthAnythingV3        # Build only"
    echo "  $0 plugins/depth_da3 DepthAnythingV3 -i     # Build and install"
...
# Check if we're in the AIFX root directory
check_openfx_root() {
    if [[ ! -f "$REPO_ROOT/CMakeLists.txt" ]] || [[ ! -d "$REPO_ROOT/plugins" ]]; then
        log_error "Not in AIFX root directory or REPO_ROOT not set correctly"
...
check_plugin_exists() {
    ...
    if [[ ! -d "$full_path" ]]; then
        ...
        log_info "Make sure the path is relative to the AIFX root directory"
        exit 1
    fi
...
clean_build() {
    if [[ "$CLEAN_BUILD" == true ]]; then
        ...
        if [[ -d "$REPO_ROOT/build" ]]; then
            rm -rf "$REPO_ROOT/build/$BUILD_TYPE"
...
configure_cmake() {
    ...
    BUILD_START_MARKER="$REPO_ROOT/build/$BUILD_TYPE/.build-start"
    ...
    # Conan APPENDS an include line to the repo-root CMakeUserPresets.json on
    # every `conan install`, and every generated CMakePresets.json declares the
    # same preset name ("conan-release"). Run the universal build once — it
    # configures under build/arm64 and build/x86_64 — and the root file ends up
    # including three files that all define "conan-release", after which every
    # subsequent `cmake --preset` fails with:
    #
    #     CMake Error: Could not read presets from ...: Duplicate preset: "conan-release"
    #
    # i.e. the documented universal-build workflow poisons the plain build for
    # everyone who runs it. The file is Conan-generated and git-ignored, so it is
    # ours to repair: keep only the include that belongs to the build directory
    # we are about to configure. Conan rewrites it on the next install anyway.
    local user_presets="$REPO_ROOT/CMakeUserPresets.json"
    local want_include="build/$BUILD_TYPE/generators/CMakePresets.json"
    if [[ -f "$user_presets" ]] && command -v python3 &>/dev/null; then
        python3 - "$user_presets" "$want_include" <<'PRUNE' || log_warning "Could not prune CMakeUserPresets.json; continuing."
import json, sys
path, keep = sys.argv[1], sys.argv[2]
with open(path) as fh:
    data = json.load(fh)
includes = data.get("include", [])
if keep in includes and len(includes) > 1:
    data["include"] = [keep]
    with open(path, "w") as fh:
        json.dump(data, fh, indent=1)
    print(f"[INFO] Pruned {len(includes) - 1} stale preset include(s) from CMakeUserPresets.json")
PRUNE
    fi
...
find_plugin_binary() {
    ...
    local target_subdir="$REPO_ROOT/$search_base/$PLUGIN_DIR/CMakeFiles/$TARGET_NAME.dir"
    if [[ ! -d "$target_subdir" || ! "$target_subdir" -nt "$BUILD_START_MARKER" ]]; then
        ...
        local fresh_targets
        fresh_targets=$(find "$REPO_ROOT/$search_base/$PLUGIN_DIR/CMakeFiles" \
    ...
    local candidate_paths=(
        "$REPO_ROOT/$search_base/$PLUGIN_DIR/$TARGET_NAME.ofx"
        "$REPO_ROOT/$search_base/$PLUGIN_DIR/$TARGET_NAME.ofx/Contents/MacOS/$TARGET_NAME"
        "$REPO_ROOT/$search_base/Examples/$(basename "$PLUGIN_DIR")/$TARGET_NAME.ofx"
        "$REPO_ROOT/$search_base/Support/Plugins/$(basename "$PLUGIN_DIR")/$TARGET_NAME.ofx"
    )
    local plugin_binary=""
...
# File mtime as a Unix timestamp. BSD stat (macOS) and GNU stat (Linux) disagree
# on flags, so probe once rather than assuming a platform.
_mtime() {
...
locate_or_create_bundle() {
    ...
    #
    # There can be MORE THAN ONE match under build/$BUILD_TYPE: the universal
    # build stages bundles at build/$BUILD_TYPE/<name>.ofx.bundle while the plain
    # build emits build/$BUILD_TYPE/$BUILD_TYPE/<name>.ofx.bundle. `find | head -1`
    # picked whichever the filesystem happened to return first, so a months-old
    # bundle left over from an earlier universal build could silently win — and
    # then get verified, installed and shipped in place of what was just compiled.
    # That is the same failure mode as a stale bundle on a workstation, only
    # upstream of it. Require the bundle to be at least as new as the binary we
    # just built, and take the newest candidate that qualifies.
    local cmake_bundle=""
    local newest_ts=0
    while IFS= read -r candidate; do
        [[ -d "$candidate/Contents" ]] || continue
        local cbin
        cbin=$(find "$candidate/Contents" -name "*.ofx" -type f 2>/dev/null | head -1)
        [[ -n "$cbin" ]] || continue
        local cts
        cts=$(_mtime "$cbin")
        # Same second counts as fresh: the bundle is a POST_BUILD copy of the binary.
        if [[ "$cts" -ge "$(_mtime "$plugin_binary")" && "$cts" -gt "$newest_ts" ]]; then
            newest_ts="$cts"
            cmake_bundle="$candidate"
        fi
    done < <(find "$REPO_ROOT/build/$BUILD_TYPE" -name "${TARGET_NAME}.ofx.bundle" -type d 2>/dev/null)

    if [[ -n "$cmake_bundle" ]]; then
    ...
    if find "$REPO_ROOT/build/$BUILD_TYPE" -name "${TARGET_NAME}.ofx.bundle" -type d 2>/dev/null | grep -q .; then
        log_warning "Found ${TARGET_NAME}.ofx.bundle under build/$BUILD_TYPE, but every copy is" >&2
        log_warning "older than the binary just built — ignoring them as stale. Run with --clean" >&2
        log_warning "if this repeats." >&2
    fi
    ...
    local bundle_path="$REPO_ROOT/build/$BUILD_TYPE/$bundle_name"
...
verify_binary_portability() {
    ...
    _verify_ofx_abi "$bundle_path"
}

# Verify the bundle carries a build stamp, and that the stamp agrees with the
# source tree it was just built from.
#
# The compile-time guard in plugins/common/aifx_abi_check.h already makes an
# ABI-mismatched plugin unbuildable, so this is the *artifact-side* half: it
# proves the identity actually reached the packaged bundle (binary marker,
# Contents/Resources/aifx-build.txt, and Info.plist all agreeing), which is what
# lets anyone identify a plugin found on a workstation months later.
_verify_ofx_abi() {
    local bundle_path="$1"
    local verifier="$SCRIPT_DIR/verify-ofx-abi.sh"

    if [[ ! -x "$verifier" ]]; then
        log_warning "tools/verify-ofx-abi.sh not found; skipping ABI/identity check."
        return
    fi

    log_info "Verifying OFX ABI + build identity"
    if ! "$verifier" --quiet "$bundle_path"; then
        log_error ""
        log_error "The bundle's build stamp is missing or inconsistent. A bundle with no"
        log_error "identity cannot be told apart from a stale install later -- which is how a"
        log_error "pre-0.1.5 macOS bundle kept segfaulting Flare long after the fix shipped."
        exit 1
    fi
    log_success "OFX ABI + build identity check passed"
}
